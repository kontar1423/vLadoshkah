...
const ShelterProfile = () => {
  ...
  const saveScrollPosition = () => {
    ...
    sessionStorage.setItem('shelterProfileScrollPosition', scrollPosition.toString());
  };

  // Восстанавливаем позицию скролла после изменения страницы
  useEffect(() => {
    const savedPosition = sessionStorage.getItem('shelterProfileScrollPosition');
    ...
    <div className="min-h-screen bg-green-95 py-4 md:py-8">
      ...
      <div className="max-w-container mx-auto px-4 sm:px-6 md:px-8 space-y-6 sm:space-y-8">
        ...
        <section className="w-full max-w-[1260px] mx-auto">
          {allPets.length > 0 ? (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-3 md:gap-6">
                {currentPets.map((pet) => (
                  <PetCard 
                    key={pet.id} 
                    petData={pet}
